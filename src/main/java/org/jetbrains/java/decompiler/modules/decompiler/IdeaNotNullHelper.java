...
public class IdeaNotNullHelper {
  ...
  private static boolean findAndRemoveParameterCheck(Statement stat, StructMethod mt) {
    ...
    if (st.type == Statement.TYPE_IF) {
      ...
      if (ifbranch != null && if_condition.type == Exprent.EXPRENT_FUNCTION
          && ((FunctionExprent) if_condition).getFuncType() == FunctionExprent.FUNCTION_EQ && ifbranch.type == Statement.TYPE_BASICBLOCK
          && ifbranch.getExprents().size() == 1 && ifbranch.getExprents().get(0).type == Exprent.EXPRENT_EXIT) {
        ...
        if (second_param.type == Exprent.EXPRENT_CONST && second_param.getExprType().type == CodeConstants.TYPE_NULL) { // TODO: reversed parameter order
  ...
  private static boolean findAndRemoveReturnCheck(Statement stat, StructMethod mt) {
    ...
    StructAnnotationAttribute attr = (StructAnnotationAttribute) attributes
        .getWithKey(StructGeneralAttribute.ATTRIBUTE_RUNTIME_INVISIBLE_ANNOTATIONS);
    if (attr != null) {
    ...
    if (parent != null && parent.type == Statement.TYPE_IF && stat.type == Statement.TYPE_BASICBLOCK && stat.getExprents().size() == 1) {
          ...
          if (ifparent.getElsestat() == stat && if_condition.type == Exprent.EXPRENT_FUNCTION
            ...
            if (second_param.type == Exprent.EXPRENT_CONST && second_param.getExprType().type == CodeConstants.TYPE_NULL) { // TODO: reversed parameter order
              ...
              if (first_param.equals(exprent_value)) { // TODO: check for absence of side effects like method invocations etc.
                if (ifbranch.type == Statement.TYPE_BASICBLOCK && ifbranch.getExprents().size() == 1 &&
      ...
      if (exprent.type == Exprent.EXPRENT_EXIT) {
        ...
        if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
          ...
          if (sequence_stats_number > 1 && sequence.getStats().getLast() == stat
