...
public class FlattenStatementsHelper {
          ...
          for (Statement st : stat.getStats()) {
            ...
            if (stat.type == Statement.TYPE_CATCHALL && ((CatchAllStatement) stat).isFinally()) {
              ...
              if (st == stat.getFirst()) { // catch head
                ...
                stack.add(
                    new StackEntry((CatchAllStatement) stat, Boolean.TRUE, StatEdge.TYPE_BREAK, root.getDummyExit(), st, st, firstnd, firstnd, true));
              }
            }
        ...
        case Statement.TYPE_ROOT:
          ...
          if (statementBreakIndex <= statsize) {
            ...
            for (int i = statementBreakIndex; i < statsize; i++) {
              ...
              lstStackStatements.addFirst(statEntry);
              lstStackStatements.addFirst(new StatementStackEntry(stat.getStats().get(i), stackFinally,
              ...
              if (entry.state) { // finally handler statement
                ...
                if (!catchall.containsStatementStrict(destination)) {
                  ...
                  stack.add(new StackEntry(catchall, Boolean.TRUE, edgetype, destination, catchall.getHandler(),
                      finallyLongRangeEntry == null ? catchall.getHandler() : finallyLongRangeEntry, sourcenode, finallyLongRangeSource, false));
  ...
  private void saveEdge(DirectNode sourcenode, Statement destination, int edgetype, DirectNode finallyShortRangeSource,
    ...
    if (finallyShortRangeSource != null) {
      ...
      lst.add(
          new String[] { finallyLongRangeSource.id, destination.id.toString(), finallyLongRangeEntry.id.toString(), isContinueEdge ? "1" : null });
    }
  }
  ...
  private static class StackEntry {
    ...
    public StackEntry(CatchAllStatement catchstatement, boolean state, int edgetype, Statement destination, Statement finallyShortRangeEntry,
        Statement finallyLongRangeEntry, DirectNode finallyShortRangeSource, DirectNode finallyLongRangeSource, boolean isFinallyExceptionPath) {
